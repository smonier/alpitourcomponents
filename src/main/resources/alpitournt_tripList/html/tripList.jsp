...
<template:addResources type="css" resources="ui.css"/>
...
<jcr:nodeProperty node="${currentNode}" name="maxtrip" var="maxtrip"/>
...
<c:choose>
    <c:when test="${empty filter.string}">
        <c:set var="querytripStatement"
               value="select * from [alpitournt:tripCard] as trip where ISDESCENDANTNODE(trip,'${currentNode.resolveSite.path}') order by trip.[tripTitle] asc "/>
    </c:when>
    <c:otherwise>
        <c:set var="querytripStatement"
               value="select * from [alpitournt:tripCard] as trip where ISDESCENDANTNODE(trip,'${currentNode.resolveSite.path}') and trip.[j:defaultCategory]='${filter.string}' order by trip.[tripTitle] asc"/>
    </c:otherwise>
</c:choose>
<jcr:sql var="tripList"
         sql="${querytripStatement}"
         limit="${maxtrip.long}"/>
                        ...
                        <div class="card-body">
                            ...
                            <ul class="list-menu">
                                ...
                                <script language="JavaScript">
                                    var categories = [];
                                    <c:forEach items="${tripList.nodes}" var="trip" varStatus="status">
                                    <jcr:nodeProperty node="${trip}" var="tripCategories" name="j:defaultCategory"/>
                                    <c:if test="${!empty tripCategories}">
                                    <c:forEach items="${tripCategories}" var="category">
                                ...
                                <script language="JavaScript">
                                    var tags = [];
                                    <c:forEach items="${tripList.nodes}" var="trip" varStatus="status">
                                    <jcr:nodeProperty node="${trip}" var="tripTags" name="j:tagList"/>
