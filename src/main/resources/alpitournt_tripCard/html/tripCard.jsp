...
<jcr:nodeProperty node="${currentNode}" name="j:tagList" var="tripTags" />
...
<c:if test="${!empty tripTags }">
    <c:forEach items="${tripTags}" var="tag">
...
<div class="card mb-4 ${myVar} ${myTags}" style="max-width:350px">
    ...
    <div class="view overlay">
        <c:choose>
            <c:when test="${jcr:isNodeType(renderContext.site, 'cldin:configuration')}">
                ...
                <img src="<cl:url node='${image.node}' width="768" gravity="${gravity}" crop="${crop}" raw="${raw}"/>"
                     srcset="<cl:url node="${image.node}" width="256" crop="${crop}" gravity="${gravity}" raw="${raw}"/> 256w,
                            <cl:url node="${image.node}" width="512" gravity="${gravity}" crop="${crop}" raw="${raw}"/> 512w,
                            <cl:url node="${image.node}" width="768" gravity="${gravity}" crop="${crop}" raw="${raw}"/> 768w,
                            <cl:url node="${image.node}" width="1024" gravity="${gravity}" crop="${crop}" raw="${raw}"/> 1024w,
                            <cl:url node="${image.node}" width="1280" gravity="${gravity}" crop="${crop}" raw="${raw}"/> 1280w,
                            <cl:url node="${image.node}" width="1600" gravity="${gravity}" crop="${crop}" raw="${raw}"/> 1600w,
                            <cl:url node="${image.node}" width="2000" gravity="${gravity}" crop="${crop}" raw="${raw}"/> 2000w"
                     class="card-img-top" style="max-height:200px"
                     alt="${fn:escapeXml(not empty tripTitle.string ? tripTitle.string : currentNode.name)}"
                />
